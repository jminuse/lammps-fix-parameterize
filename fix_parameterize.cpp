...
void FixParameterize::init()
{
  ...
  id = modify->find_compute("sum_pe"); //must use this name for the compute!
  if (id < 0) error->all(FLERR,"Minimization could not find reduce compute (must be named sum_pe)");
  compute_sum_pe = modify->compute[id];
...
{
  //update->eflag_atom = update->ntimestep; //to prevent "ERROR: Per-atom energy was not tallied on needed timestep (../compute_pe_atom.cpp:84)". Maybe not the right way to do this?
  ...
  for(unsigned int i=0; i<atom->natoms; i++) {
    ...
    if(which_system>=current_energies.size()) {
      printf("Index %u in vector of size %lu: ", which_system, current_energies.size());
      error->all(FLERR,"Invalid mapping of atom to system");
    }
    //printf("Atom index=%d, PE invoked=%d, PE vector=%p, PE=%g\n", i, compute_pe->invoked_flag, compute_pe->vector_atom, compute_pe->vector_atom[i]);
