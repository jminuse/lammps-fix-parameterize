...
void MinParams::write_tersoff_file() {
  ...
  //label
  fprintf(output, "# Generated by min_style params. LAMMPS units = real.\n");
  //output error
    ...
    for(int j_type = 1; j_type <= atom->ntypes; j_type++) {
      for(int k_type = 1; k_type <= atom->ntypes; k_type++) {
        ...
        if(tersoff->params[index_iii].biga > 0.0) { // mix iii and jjj parameters
          ...
          printf("Bonds: %s--%s--%s = mix(%s--%s--%s, %s--%s--%s)  ", element_j, element_i, element_k,    element_j, element_i, element_j,    element_k, element_i, element_k);
